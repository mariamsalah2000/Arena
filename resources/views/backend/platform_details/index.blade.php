...
@section('content')
    ...
    <div class=" text-left mt-2 mb-3">
        <div class="align-items-center">
            <h1 class="h3">Platform Details</h1>
        </div>
    </div>
    ...
    <div class="card">
        <form class="" id="sort_customers" action="" method="GET">
            <div class="card-header row gutters-5">
                <div class="col">
                    <h5 class="mb-0 h6">Platform Details</h5>
                </div>
                    ...
                    <tbody>
                        @foreach ($details as $key => $detail)
                            @if ($platform != null)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td style="width: 25%"><img style="width: 100%" src="{{ asset($detail->img) }}">
                                    </td>
                                    <td style="width: 25%">{{ $detail->title }}</td>
                                    <td style="width: 25%">{{ $detail->desc }}</td>
                                    <td style="width: 25%">{{ $detail->platform->title }}</td>

                                    <td>{{ $platform->order }}</td>
                                    <td class="text-right">
                                        <a href="{{ route('platform_details.edit', $detail->id) }}"
                                            class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                                            title="{{ trans('all.Edit') }}">
                                            <i class="las la-edit"></i>
                                        </a>
                                        <a href="{{ route('platform_details.destroy', $detail->id) }}"
                                            class="btn btn-soft-primary btn-icon btn-circle btn-sm"
