...
<div class=" text-left mt-2 mb-3">
    <div class="align-items-center">
        <h1 class="h3">{{trans('all.All Accessories')}}</h1>
    </div>
</div>
...
<div class="card">
    <form class="" id="sort_customers" action="" method="GET">
        <div class="card-header row gutters-5">
            <div class="col">
                <h5 class="mb-0 h6">{{trans('all.Accessories')}}</h5>
            </div>
                ...
                <tbody>
                    @foreach($accessories as $key => $accessory)
                        @if ($accessory != null)
                            <tr>
                                @if($accessory->cover)
                                    <td style="width: 25%"><img style="width: 100%" src="{{asset($accessory->cover)}}"></td>
                                @else
                                ...
                                <td class="text-right">
                                    <a href="{{route('accessories.edit', $accessory->id)}}" class="btn btn-soft-primary btn-icon btn-circle btn-sm" title="{{ trans('all.Edit') }}">
                                            <i class="las la-edit"></i>
                                    </a>
                                    <a href="{{route('accessories.destroy', $accessory->id)}}" class="btn btn-soft-primary btn-icon btn-circle btn-sm" title="{{ trans('all.Delete') }}">
